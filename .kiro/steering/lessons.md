...
- **Revert một PR rồi merge lại nhánh đó để "lấy code về"** → SAI. Git không phục hồi: merge chỉ so với merge-base nên phần đã revert mất vĩnh viễn. Đây là lý do `dev` mất sạch `packages/` và `app/src/lib/`. Phải revert chính commit revert, hoặc cherry-pick lại.
- **`git rebase <đích>` khi nhánh nền có commit phá hoại** → SAI, kéo theo cả commit của nhánh nền. Dùng `git rebase --onto <đích> <nền> <nhánh>` để replay chỉ commit của mình.
